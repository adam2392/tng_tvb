...
def converttimestowindow(timepoints, times):
    ...
    if times.size > 1:
        for time in times:
            timeindex = np.where(np.logical_and(timepoints[:,0] < time, timepoints[:,1] > time))[0]
            if len(timeindex) >= 1:
                winindices.append(timeindex[0])
    else:
        winindices.append(np.where(np.logical_and(timepoints[:,0] < times, timepoints[:,1] > times))[0][0])
...
def returnindices(pat_id, seiz_id=None):
    included_indices, onsetelecs, resectelecs, clinresult = returnnihindices(pat_id, seiz_id)
    if included_indices.size == 0:
        included_indices, onsetelecs, resectelecs, clinresult = returnlaindices(pat_id, seiz_id)
    if included_indices.size == 0:
        included_indices, onsetelecs, resectelecs, clinresult = returnummcindices(pat_id, seiz_id)
    if included_indices.size == 0:
        included_indices, onsetelecs, resectelecs, clinresult = returnjhuindices(pat_id, seiz_id)
    if included_indices.size == 0:
        included_indices, onsetelecs, resectelecs, clinresult = returntngindices(pat_id, seiz_id)
...
def returnlaindices(pat_id, seiz_id):
    ...
    if pat_id == 'la01':
        ...
        included_indices = np.concatenate((np.arange(2,3), np.arange(6,17),
             ...
             np.arange(50,56), np.arange(57,62), np.arange(63,69),
