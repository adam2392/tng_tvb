...
def _peakdetect_parabola_fitter(raw_peaks, x_axis, y_axis, points):
    ...
    """
    def func(x, a, tau, c): return a * ((x - tau) ** 2) + c
    fitted_peaks = []
...
def peakdetect_fft(y_axis, x_axis, pad_len=20):
    ...
    def padd(x, c): return x[:len(x) // 2] + [0] * c + x[len(x) // 2:]

    def n(x): return int(log(x) / log(2)) + 1
...
def peakdetect_sine(y_axis, x_axis, points=31, lock_frequency=False):
    ...
    if lock_frequency:
        def func(x_ax, A, tau): return A * np.sin(
            2 * pi * Hz * (x_ax - tau) + pi / 2)
    else:
        def func(x_ax, A, Hz, tau): return A * np.sin(
            2 * pi * Hz * (x_ax - tau) + pi / 2)
...
def peakdetect_spline(y_axis, x_axis, pad_len=20):
    ...
    # get peaks
    max_peaks, min_peaks = peakdetect_zero_crossing(
...
def zero_crossings_sine_fit(y_axis, x_axis, fit_window=None, smooth_window=11):
    ...
    def offset_func(x, k, m): return k * x + m
    k = []
    m = []
    amplitude = []
    ...
    def func(x, tau): return A * np.sin(2 * pi * Hz *
                                        (x - tau)) + offset_func(x, *p_offset)
